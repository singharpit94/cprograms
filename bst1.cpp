...
void preorder(struct node *ptr,int);
void inorder(struct node *ptr);
void postorder(struct node *ptr,int);
...
void preorder(struct node *ptr,int flag=0)
{
	...
	{   
		for(int i=0;i<indent;i++)
		{
		     ...
		     cout<<"|\n";
	 	      ...
	 	      cout<<setw(indent/pow(2,p))<<"|\n";
	     }
	     if(indent==2)
	     cout<<ptr->info;
	     else
		cout<<setw(indent/pow(2,p))<<ptr->info;
		
	}
	
     indent+=2;
     p+=1;
	preorder(ptr->lchild,0);
	indent+=2;
	p+=1;
	preorder(ptr->rchild,1);
}/*End of preorder( )*/
...
void postorder(struct node *ptr,int indent =0)
{
	if(ptr == NULL )/*Base Case*/
		return;
	...
	if (indent) {
        ...
        printf("%d \n",ptr->info) ;
        postorder(ptr->lchild,indent+4);
	postorder(ptr->rchild,indent+4);
